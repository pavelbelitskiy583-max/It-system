...
function shapeUser(u) {
  return {
    id: u.id, login: u.login, name: u.name, role: u.role, branchId: u.branch_id,
...
async function authRegister(req, res) {
  const { orgName, adminName, login, password } = req.body || {};
  ...
  await sql`INSERT INTO users (id, org_id, login, name, role, permissions, pass_hash, must_change_password)
            VALUES (${userId}, ${orgId}, ${login.trim()}, ${adminName.trim()}, 'admin', ${JSON.stringify(fullPermissions())}::jsonb, ${hash}, false)`;
